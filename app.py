...
st.markdown("Upload your Master Excel. The app will generate a formatted **Table 1** for every town found in the data.")

uploaded_file = st.file_uploader("Upload Master Excel", type=['xlsx'])

# List of columns we absolutely need
REQUIRED_COLUMNS = [
    'Town', 'Location / T2T', 'Updated Stratification', 'TVS Store Type', 
    'BAL Store Type', 'S1 Ind - F Vistaa', 'BAL S1 Vol - Vistaa', 
    'TVS S1 Vol Basis MS', 'Network Intervention', 'Pre - Network - BAL', 'Post Net Bal'
]
...
if uploaded_file:
    ...
    # CRITICAL FIX: Clean column names to prevent KeyError
    master_df.columns = [str(c).strip() for c in master_df.columns]
    
    # Check if all required columns are present
    missing = [col for col in REQUIRED_COLUMNS if col not in master_df.columns]
    
    if missing:
        st.error(f"Missing columns in Excel: {missing}")
        st.info(f"Available columns: {list(master_df.columns)}")
    else:
        towns = master_df['Town'].dropna().unique()
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            for town in towns:
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    ...
                    strat_fmt = workbook.add_format({'align': 'center', 'valign': 'vcenter', 'border': 1, 'bold': True, 'bg_color': '#F2F2F2'})
                    ...
                    worksheet.set_column('C:S', 12, num_fmt)
                    worksheet.set_column('J:J', 12, pct_fmt)
                
                zip_file.writestr(f"{town}_Table_1.xlsx", output.getvalue())

        st.success(f"Successfully processed {len(towns)} towns!")
        st.download_button(
            label="📥 Download All Towns (ZIP)",
            data=zip_buffer.getvalue(),
            file_name="Network_Analysis_Reports.zip",
